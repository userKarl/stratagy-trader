package com.zd.business.entity.ctp;
...
public class Order implements Serializable {
	...
	public String MyToString() {
		try {
			...
			String totalVolume = String.valueOf(this.totalVolume);
			String tradedVolume = String.valueOf(this.tradedVolume);
			String direction="";
			if (price.endsWith(dotZero)) {
				price = price.substring(0, price.indexOf(dotZero));
			}
			if (totalVolume.endsWith(dotZero)) {
				totalVolume = totalVolume.substring(0, totalVolume.indexOf(dotZero));
			}
			if (tradedVolume.endsWith(dotZero)) {
				tradedVolume = tradedVolume.substring(0, tradedVolume.indexOf(dotZero));
			}
			if(BuySaleEnum.BUY.getCode().equals(this.direction)) {
				direction=RtConstant.DIRECTION_LONG;
			}else if(BuySaleEnum.SALE.getCode().equals(this.direction)) {
				direction=RtConstant.DIRECTION_SHORT;
			}else {
				direction=this.direction;
			}
			
			return String.join("@", Lists.newArrayList(this.gatewayID, this.symbol, this.exchange, this.rtSymbol,
					this.orderID, this.rtOrderID, direction, this.offset, price, totalVolume, tradedVolume,
					this.status, this.tradingDay, this.orderDate, this.orderTime, this.cancelTime, this.activeTime,
					this.updateTime, String.valueOf(this.frontID), String.valueOf(this.sessionID), this.priceType));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
