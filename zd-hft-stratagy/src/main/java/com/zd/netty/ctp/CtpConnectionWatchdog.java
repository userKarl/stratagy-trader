...
public class CtpConnectionWatchdog extends ConnectionWatchdog {

	private static final Logger logger = LoggerFactory.getLogger(CtpConnectionWatchdog.class);
	...
	public CtpConnectionWatchdog(Bootstrap boot, Timer timert, String host, int port) {
		this.bootstrap = boot;
		this.timer = timert;
		this.host = host;
		this.port = port;
	...
	public void channelActive(final ChannelHandlerContext ctx) throws Exception {
		NettyGlobal.ctpServerChannalMap.put(NettyGlobal.CTPSERVERCHANNELKEY, ctx);
		channel = ctx.channel();
		attempts = 0;
		reconnect = true;
		if (!Check) {
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		NettyGlobal.ctpServerChannalMap.remove(NettyGlobal.CTPSERVERCHANNELKEY, ctx);
		disConnect = true;
		...
		try {
			...
			if(!CommandCode.HEARTBIT.equals(ni.code)) {
				...
				if (StringUtils.isNotBlank(ni.infoT) && StringUtils.isNotBlank(ni.code)
						&& CommandCode.MARKET02.equals(ni.code)) {
