...
public class LoginResponseInfo4Front2 implements NetParent {
	...
	public String MyToString() {
		String temp = this.userId + "@" + this.userName + "@" + this.userType
				...
				+ "@" + this.ICEUSMarket + "@" + this.ICEECMarket + "@" + this.ICEEFMarket
				+ "@" + this.FCanTradeStockHK + "@" + this.FCanTradeStockAM + "@" + this.FMultiLogin
				+ "@" + this.FSellStockHK + "@" + this.FSellStockAM + "@" + this.FCanTradeStockKRX
				+ "@" + this.FHKEXMarket + "@" + this.FIDNumber + "@" + this.FHKEXMarketFee
				+ "@" + this.FISProfessional + "@" + this.FISOverSea + "@" + this.isFirstLogin
				+ "@" + this.userMobile + "@" + this.hasSetQA + "@" + this.FCanTradeStockSGXQ
				+ "@" + this.existMac;
	...
	public void MyReadString(String temp) {
		...
		if (arrClass.length > 24) {
			this.FCanTradeStockHK = arrClass[24];
		}
		if (arrClass.length > 25) {
			this.FCanTradeStockAM = arrClass[25];
		}
		if (arrClass.length > 26) {
			this.FMultiLogin = arrClass[26];
		}
		if (arrClass.length > 27) {
			this.FSellStockHK = arrClass[27];
		}
		if (arrClass.length > 28) {
			this.FSellStockAM = arrClass[28];
		}
		if (arrClass.length > 29) {
			this.FCanTradeStockKRX = arrClass[29];
		}
		if (arrClass.length > 30) {
			this.FHKEXMarket = arrClass[30];
		}
		if (arrClass.length > 31) {
			this.FIDNumber = arrClass[31];
		}
		if (arrClass.length > 32) {
			this.FHKEXMarketFee = Double.parseDouble(arrClass[32].trim().equals("") ? "0" : arrClass[32]);
		}
		if (arrClass.length > 33) {
			this.FISProfessional = arrClass[33];
		}
		if (arrClass.length > 34) {
			this.FISOverSea = arrClass[34];
		}
		if (arrClass.length > 35) {
			this.isFirstLogin = arrClass[35];
		}
		if (arrClass.length > 36) {
			this.userMobile = arrClass[36];
		}
		if (arrClass.length > 37) {
			this.hasSetQA = arrClass[37];
		}
		if (arrClass.length > 38) {
			this.FCanTradeStockSGXQ = arrClass[38];
		}
		if (arrClass.length > 39) {
			this.existMac = arrClass[39];
		}
	}
	...
	public String MyPropToString() {
		String temp = "userId@userName@userType@loginPwd@accountNo"
				...
				+ "@ICEUSMarket@ICEECMarket@ICEEFMarket@FCanTradeStockHK@FCanTradeStockAM@FMultiLogin"
				+ "@FSellStockHK@FSellStockAM@FCanTradeStockKRX@FHKEXMarket@FIDNumber@FHKEXMarketFee"
				+ "@FISProfessional@FISOverSea@isFirstLogin@userMobile@hasSetQA@FCanTradeStockSGXQ"
				+ "@existMac";
		return temp;
	}
}
