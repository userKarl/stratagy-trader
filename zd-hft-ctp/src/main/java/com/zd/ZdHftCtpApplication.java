...
public class ZdHftCtpApplication implements CommandLineRunner {
	...
	public ResultVO sendOrder(@RequestBody JSONObject jsonObject) {
		...
		if (StringUtils.isEmpty(gatewayID) || StringUtils.isEmpty(rtSymbol) || StringUtils.isEmpty(priceType)
				|| StringUtils.isEmpty(direction) || StringUtils.isEmpty(offset) || volume <= 0) {
